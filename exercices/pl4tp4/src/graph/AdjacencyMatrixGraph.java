...
public class AdjacencyMatrixGraph<V, E> implements BasicGraph<V, E>, Cloneable {
    ...
    public Iterable<V> directConnections(V vertex) {
        ...
        if (position != -1) {
            LinkedList<V> connections = new LinkedList<>();

            for (int otherVertex = 0; otherVertex < this.numVertices; otherVertex++) {
                if (this.privateGet(position, otherVertex) != null) {
                    connections.add(this.vertices.get(otherVertex));
                }
            }
    ...
    public Iterable<E> outgoingEdges(V vertex) {
        ...
        if (position != -1) {
            ...
            for (int otherVertex = 0; otherVertex < this.numVertices; otherVertex++) {
                if (this.edgeMatrix[position][otherVertex] != null) {
                    edges.add(this.edgeMatrix[position][otherVertex]);
                }
            }
